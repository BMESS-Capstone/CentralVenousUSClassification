...
def generalized_dice_score(y_true, y_pred):
    ...
    for i in range(0, y_pred.shape[-1]):
        intersection_sum = tf.reduce_sum(input_tensor=intersection[..., i])
        union_sum = tf.reduce_sum(input_tensor=union[..., i])
        class_weight = 1.0 / (tf.reduce_sum(input_tensor=y_true[..., i]) ** 2 + epsilon)
...
def recall(y_true, y_pred):
    ...
    for i in range(0, num_classes):
        true_pos = tf.reduce_sum(input_tensor=y_true[..., i] * y_pred[..., i])
        true_neg = tf.reduce_sum(input_tensor=(1 - y_true[..., i]) * (1 - y_pred[..., i]))
        false_neg = tf.reduce_sum(input_tensor=1 - y_pred[..., i]) - true_neg
...
def specificity(y_true, y_pred):
    ...
    for i in range(0, num_classes):
        true_pos = tf.reduce_sum(input_tensor=y_true[..., i] * y_pred[..., i])
        true_neg = tf.reduce_sum(input_tensor=(1 - y_true[..., i]) * (1 - y_pred[..., i]))
        false_pos = tf.reduce_sum(input_tensor=y_pred[..., i]) - true_pos
